...
parking1_start = (100, 100)
parking1_end = (300, 300)  # Example size: 200x200
...
stabilization_threshold = 5  # Number of consecutive frames required to confirm a status change
...
while dpg.is_dearpygui_running():
    ...
    for i, (start, end, status_tag) in enumerate(parking_areas, start=1):
        ...
        cars = car_cascade.detectMultiScale(gray_parking, scaleFactor=1.1, minNeighbors=5, minSize=(30, 30))
        ...
        if len(cars) > 0:
            ...
            color = (0, 0, 255)
        elif percent_change > 20:  # Increase sensitivity threshold
        ...
        # Check for delayed transition from "Occupied" to "Obstructed"
        if previous_status[status_tag] == "Occupied" and current_status == "Obstructed":
            delay_counters[status_tag] += 1
            if delay_counters[status_tag] < 60:  # Delay for ~2 seconds (assuming 30 FPS)
                current_status = "Occupied"
                color = (0, 0, 255)
            else:
                delay_counters[status_tag] = 0  # Reset delay counter
