...
while dpg.is_dearpygui_running():
    ...
    if ref_frame is None:
        cv2.imshow("Parking Scanner", frame)  # Show the camera feed even if no reference frame is set
        if key == ord('q'):
            break
    ...
    for i, (start, end, status_tag) in enumerate(parking_areas, start=1):
        ...
        cars = car_cascade.detectMultiScale(gray_parking, scaleFactor=1.1, minNeighbors=3, minSize=(30, 30))

        # Check for changes in the parking area
        diff = cv2.absdiff(ref_frame[start[1]:end[1], start[0]:end[0]], parking)
        ...
        for status in stabilization_counters[status_tag]:
            if status == current_status:
                ...
                stabilization_counters[status_tag][status] = 0
