...
stabilization_counters = {status_tag: {"Occupied": 0, "Vacant": 0, "Obstructed": 0} for _, _, status_tag in parking_areas}
stabilization_threshold = 10  # Require 10 consecutive frames
...
delay_counters = {status_tag: 0 for _, _, status_tag in parking_areas}  # Initialize delay counters

# Initialize background subtractor
bg_subtractor = cv2.createBackgroundSubtractorMOG2(history=1000, varThreshold=250, detectShadows=True)
...
while dpg.is_dearpygui_running():
    ...
    for i, (start, end, status_tag) in enumerate(parking_areas, start=1):
        ...
        # Preprocess the parking area
        gray_parking = cv2.cvtColor(parking, cv2.COLOR_BGR2GRAY)
        gray_parking = cv2.equalizeHist(gray_parking)  # Improve contrast
        gray_parking = cv2.GaussianBlur(gray_parking, (5, 5), 0)  # Reduce noise

        # Detect cars in the parking area
        cars = car_cascade.detectMultiScale(
            gray_parking,
            scaleFactor=1.05,  # Lower value for finer scaling
            minNeighbors=3,    # Reduce to detect more objects
            minSize=(30, 30)   # Adjust based on expected car size
        )

        # Debugging: Print detected cars and draw rectangles
        print(f"Detected cars: {len(cars)}")
        for (x, y, w, h) in cars:
            cv2.rectangle(parking, (x, y), (x + w, y + h), (255, 0, 0), 2)
        ...
        # Crop the reference frame to match the parking area
        ref_parking = ref_frame[start[1]:end[1], start[0]:end[0]]
        gray_ref_parking = cv2.cvtColor(ref_parking, cv2.COLOR_BGR2GRAY)

        # Compute the absolute difference
        diff = cv2.absdiff(gray_ref_parking, gray_parking)
        _, motion_thresh = cv2.threshold(diff, 30, 255, cv2.THRESH_BINARY)
        motion_detected = cv2.countNonZero(motion_thresh) > 500  # Example threshold

        # Determine the current status
        current_status = "Vacant"
        color = (0, 255, 0)

        if len(cars) > 0 and motion_detected:
        ...
        if stabilization_counters[status_tag][current_status] >= stabilization_threshold:
            ...
            if previous_status[status_tag] == "Occupied" and current_status == "Vacant":
                if parking_start_time[status_tag] is not None:  # Ensure start time is set
                    ...
                    parking_start_time[status_tag] = None  # Reset start time after logging
                else:
                    print(f"Error: Parking start time for {status_tag} is not set.")
        ...
        status_history[status_tag].append(current_status)
        if len(status_history[status_tag]) > history_length:
            status_history[status_tag].pop(0)

        # Determine the most frequent status in the history
        most_frequent_status = max(set(status_history[status_tag]), key=status_history[status_tag].count)
        if most_frequent_status != previous_status[status_tag]:
            previous_status[status_tag] = most_frequent_status
